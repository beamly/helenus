...
var Connection = function(options){
  ...
  this.connNum = options.connNum || 0;
...
Connection.prototype.execute = function(){
  ...
  timer = setTimeout(function(){
    if(!calledBack){
      console.log('TIMEOUT: executed', command, 'on', self.host, args);

      calledBack = true;

      var duration = new Date() - startTime;
      if(self.emitFastQueryTimes || (self.emitSlowQueryTimes && (duration >= self.slowQueryTimeLimit))) {
        self.emit('querytime', {
          host: self.host,
          slow: (duration >= self.slowQueryTimeLimit),
          timeout: true,
          duration: duration,
          command: command,
          args: args,
      ...
      var key;
      try {
         key = new uuid.UUID.fromBinary(args[0]).hex;
      } catch (e) {
      }
      callback(errors.create({ name: 'TimeoutException', why: 'Connection Timed Out', host:self.host, command:command, key:key, args:args}));
      self._connection.connection.destroy();
    }
  }, this.timeout);
  ...
  if(process.domain) {
    args.push(process.domain.bind(onReturn));
  } else {
    args.push(onReturn);
  }
 ...
 */
Connection.prototype.describeRing = function(keyspace, callback) {
  callback = callback || NOOP;
  keyspace = keyspace || '';

  this.execute('describe_ring', keyspace, callback);
};
