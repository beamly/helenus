...
var Pool = function(options){
  ...
  if(!options.hosts && options.host){
    this.hosts = [options.host+":0"];
  } else {
    this.hosts = options.hosts.map(function(host) { return host + ":0" });
  }
...
Pool.prototype.connect = function(callback){
  ...
  function connect(host, connNum){
    ...
    connection.connect(function(err, keyspace){
      self.emit('connect-host', host + ":" + connNum);
    ...
    connection.on('querytime', function(query){
      ...
      self.clients.forEach(function(client) {
         hosts.push(client.host + ":" + client.port + ":" + client.connNum);
      });
      query.pool = {
         active: hosts,
         dead: self.dead
  ...
  function resolve(host) {
	...
	// Remove unresolved hostname from the hosts list
       	self.hosts.splice(self.hosts.indexOf(host), 1);

	// Add its IP addresses and attempt to connect
	for(var i = 0; i < addresses.length; i++) {
        	var address = addresses[i] + ":" + port;
    		for(var j=0; j < self.hostPoolSize; j +=1){
			self.hosts.push(address + ":" + j);
        		connect(address, j);
		}
	}
      } else {
    	for(var j=0; j < self.hostPoolSize; j +=1){
        	self.markHostDead(host + ":" + j);
	}
        self.emit('error', err);
      }
    });
...
Pool.prototype.suspendHost = function(conn) {
  ...
  this.markHostDead(conn.host + ':' + conn.port + ":" + conn.connNum);
};

Pool.prototype.markHostDead = function(host) {
  if(this.dead.indexOf(host) === -1) {
    this.dead.push(host);
    this.emit('suspend-host', host);
  }
};
  ...
  if (host.ready){
    ...
    for(; i < len; i += 1){
      if(this.clients[i].ready){
        ...
        this.markHostDead(this.clients[i].host + ':' + this.clients[i].port);
      }
    }
      ...
      if(err) {
        ...
        for(var r = 0; r < ring.length; r++) {
          ...
          for(var n = 0; n < range.length; n++) {
            ...
            if(self.hosts.indexOf(endpoint + ":0") === -1) {
              if(!self.nodeDiscoveryDataCenter || self.nodeDiscoveryDataCenter === node.datacenter) {
                self.emit('discovery', node);
		// Schedule host for availability check within next 5s
for(var i = 0; i < self.hostPoolSize; i++)  {
                self.dead.push(endpoint + ":" + i);
                self.hosts.push(endpoint + ":" + i);
}
              }
            }
          }
