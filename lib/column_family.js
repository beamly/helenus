...
/**
 * Gets a key range based on some options
 * @private
 * @memberOf ColumnFamily
 * @returns {KeyRange}
 */
function getKeyRange(options, serializer){
  var predicate = new ttype.SlicePredicate();

  var startKey = options.startKey != null ? serializer.serialize(options.startKey) : null;
  var endKey = options.endKey != null ? serializer.serialize(options.endKey) : null;
  var startToken = options.startToken != null ? serializer.serialize(options.startToken) : null;
  var endToken = options.endToken != null ? serializer.serialize(options.endToken) : null;

  return new ttype.KeyRange({
    start_key:startKey,
    end_key:endKey,
...
/**
 * Remove a single row or counter column
 * This function uses a variable-length paramter list.  Which parameters
 * are passed depends on which column path should be used for the
 * removal and whether this column family is a super column or not.
 *
 ...
 * @param {Function} callback The callback to call once complete (optional)
 */
ColumnFamily.prototype.removeCounter = function() {
  var args = normalizeParameters(Array.prototype.slice.apply(arguments));
  args.callback = args.callback || NOOP;
  args.options = args.options || { };
  ...
  this.connection.execute('remove_counter', marshalledKey, path, consistency, args.callback);
};

/**
 * Get rows by a range of keys
 * @param {Object} startKey the first key
 * @param {Object} endKey the last key
 ...
 */
ColumnFamily.prototype.getRangeSlice = function(startKey, endKey, max, options, callback){
  if (typeof options === 'function'){
    callback = options;
    options = {};
  }
  ...
  this.connection.execute('get_range_slices', columnParent(self), predicate, keyRange, consistency, onComplete);
};
...
ColumnFamily.prototype.multiGet = function(keys, options, callback){
  ...
  for(var i = 0; i < keys.length; i++) {
      marshalledKeys.push(this.keyMarshaller.serialize(keys[i]).toString('binary'));
  }
  ...
  function onComplete(err, val){
    ...
    for(key in val){
      row = val[key];
      key = self.keyMarshaller.deserialize(key);
      results.push(Row.fromThrift(key, row, self));
    }
    callback(null, results);
  }

  this.connection.execute('multiget_slice', marshalledKeys, columnParent(self), predicate, consistency, onComplete);
};
...
ColumnFamily.prototype.getIndexed = function(query, options, callback){
  ...
  query.start = query.start ? this.keyMarshaller.serialize(query.start) : '';
